...
template <typename String> static std::string encode_pem(String s) { return encode_with_line_breaks<String, 64>(s); }

template <typename String> static std::string encode_mime(String s) { return encode_with_line_breaks<String, 76>(s); }
...
std::string base64_encode(unsigned char const *bytes_to_encode, size_t in_len, const std::string& prefix, bool url)
{
    ...
    std::string ret;
    ret.reserve(len_encoded + prefix.length());
    ret += prefix;
...
template <typename String> static std::string decode(String const &encoded_string, bool remove_linebreaks)
{
    ...
    while (pos < length_of_string) {
        ...
        if ((pos + 2 < length_of_string) && // Check for data that is not padded with equal signs (which is allowed by RFC 2045)
            encoded_string.at(pos + 2) != '=' && encoded_string.at(pos + 2) != '.' // accept URL-safe base 64 strings, too, so check for '.' also.
        ) {
            //
